...
async function buildWebsiteHtml(prompt, options) {
    ...
    const chunks = Array.isArray(data?.candidates)
        ? data.candidates.flatMap((candidate) => Array.isArray(candidate?.content?.parts)
            ? candidate.content.parts.map((part) => (typeof part?.text === 'string' ? part.text : '')).filter(Boolean)
            : [])
        : [];
...
function cleanModelHtml(text) {
    if (!text)
        return fallbackWebsiteHtml('No content generated.', {});
...
async function deployToVercel(html, options) {
    ...
    const payload = {
        ...
        files: [
            ...
            {
                file: 'vercel.json',
                data: Buffer.from(JSON.stringify({ rewrites: [{ source: '/(.*)', destination: '/index.html' }] }, null, 2), 'utf-8').toString('base64'),
                encoding: 'base64'
            }
        ],
        projectSettings: {
